...
const schema = yup.object().shape({
  ...
  confirm_password: yup.string().required('Confirm password is required').min(7, "Password must be at least 7 characters long").oneOf([yup.ref("password"), []], "Passwords must match"),
});
...
const [email, email_atrs] = defineField('email');
const [password, password_atrs] = defineField('password');
const [confirm_password, confirm_password_atrs] = defineField('confirm_password')
    ...
    <div class="bg-surface-0 dark:bg-surface-900 p-6 shadow rounded-border w-full lg:w-6/12 mx-auto">
      <div class="text-center mb-8">
        ...
        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">
          Welcome to FlexNotes
        </div>
        <span class="text-surface-600 dark:text-surface-200 font-medium leading-normal">
          Did you already have account ?
        </span>
        <a class="font-medium no-underline ml-2 text-primary cursor-pointer" @click="router.push({ path: '/login' })">Go
          to login!</a>
      </div>
      ...
      <Form @submit="onSubmit">
        <div class="mb-4">
          ...
          <InputText email="username" id="username" v-model="username" type="text" placeholder="Username" class="w-full"
        ...
        <div class="mb-4">
          ...
          <InputText name="email" id="email" v-model="email" type="text" placeholder="E-Mail" class="w-full"
        ...
        <div class="mb-4">
          <label for="confirm_password" class="text-surface-900 dark:text-surface-0 font-medium mb-2 block">
            Confirm password
          </label>

          <InputText name="confirm_password" type="password" id="confirm_password" v-model="confirm_password"
            placeholder="Repeat password" class="w-full" :class="{ 'p-invalid': errors.confirm_password }" />

          <small class="p-error">{{ errors.confirm_password }}</small>
        </div>
